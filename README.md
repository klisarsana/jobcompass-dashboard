...
4. **🔍 Eksplorasi Master Data**: Tabel interaktif yang memungkinkan pengguna untuk mencari dan memfilter raw data spesifik berdasarkan *Job Title* (Kata Kunci Jabatan).
...
│
├── notebooks/
│   └── jobcompass.ipynb        # Jupyter Notebook untuk eksplorasi data, pembersihan, dan eksperimen model/analitik
│
...
2. **Buat Virtual Environment**:
   Karena folder `venv` tidak ikut dipublish ke *repository*, Anda perlu membuatnya terlebih dahulu:
   ```bash
   python -m venv venv
   ```
...
Selain itu, terdapat `dataset_final.csv` di direktori yang sama, yang merupakan data hasil pemrosesan akhir yang dikhususkan untuk eksperimen dan **Pelatihan Model AI**. 
...
---
*Dibuat untuk memberikan pandangan terarah (*insight-driven*) pada kondisi bursa kerja global masa kini.*
